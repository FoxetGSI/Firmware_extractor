...
if [[ ! -d "$toolsdir/extract_android_ota_payload" ]]; then
    git clone -q https://github.com/vm03/payload_dumper.git "$toolsdir/extract_android_ota_payload"
else
...
if [[ $(7z l -ba "$romzip" | grep system.new.dat) ]]; then
    ...
    python3 "$LOCALDIR/tools/extract_android_ota_payload/payload_dumper.py" payload.bin --out $tmpdir
    for partition in $PARTITIONS; do
        [[ -e "$tmpdir/$partition.img" ]] && mv "$tmpdir/$partition.img" "$outdir/$partition.img"
    done
    rm payload.bin
